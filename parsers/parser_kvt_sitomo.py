...
class Parser_kvt_sitomo:
    ...
    def get_result_list(self, data_list):
        ...
        for item in data_list:
            ...
            if len(str(item[0])) > 0 and not pd.isnull(item[5]) and item[0] != 'код товара' and len(str(item[2])) > 3:


                result_item.article = item[0]
                result_item.name = item[2]
                result_item.unit = item[3]
                result_item.quantity = item[4] if len(str(item[4])) > 0 or item[4] != 'nan' else 0
